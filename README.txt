...
Also a special thanks to Zuo Fei for providing a base for code as well as for help with the word search problem
	...
	-Run server program before you run the client program. Server must be running before client 	tries to connect, otherwise client program will exit.
	...
	into a string. Then it uses a while loop to search through the string and adds only the 		lines containing the target word to a string called result. This string is printed to the
	...
	buffer bufferOut, which is then written to the client_fd socket. After that the client_fd 		socket is closed, however to end the program one should enter ctrl+C (^C). 
	...
	only the lines containing a specific word as designated on the side of the host and 	output these lines to the console.
	
	-After the client is connected to the server, it immediately writes to the server using 		the write() call, and then writes an additional character to the end so that the host 		will know when it has reached the end of the file. Afterwards, the client waits and reads 		the socket for the host's response.
	
	-After the client receives the host's response it writes to the console upon the closing 		of the server program. 
